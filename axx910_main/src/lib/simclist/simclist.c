...
int list_init(list_t *restrict l) {
    ...
    l->head_sentinel = (struct list_entry_s *)os_malloc(sizeof(struct list_entry_s));
    l->tail_sentinel = (struct list_entry_s *)os_malloc(sizeof(struct list_entry_s));
    ...
    l->spareels = (struct list_entry_s **)os_malloc(SIMCLIST_MAX_SPARE_ELEMS * sizeof(struct list_entry_s *));
    l->spareelsnum = 0;
...
void list_destroy(list_t *restrict l) {
    ...
    for (i = 0; i < l->spareelsnum; i++) {
        os_free(l->spareels[i]);
    }
    os_free(l->spareels);
    os_free(l->head_sentinel);
    os_free(l->tail_sentinel);
}
...
int list_insert_at(list_t *restrict l, const void *data, unsigned int pos) {
    ...
    /* this code optimizes os_malloc() with a free-list */
    if (l->spareelsnum > 0) {
        lent = l->spareels[l->spareelsnum-1];
        l->spareelsnum--;
    } else {
        lent = (struct list_entry_s *)os_malloc(sizeof(struct list_entry_s));
        if (lent == NULL)
            return -1;
    }
    ...
    if (l->attrs.copy_data) {
        ...
        lent->data = (struct list_entry_s *)os_malloc(datalen);
...
int list_delete_range(list_t *restrict l, unsigned int posstart, unsigned int posend) {
    ...
    if (l->attrs.copy_data) {
        ...
        for (; i <= posend; i++) {
            tmp2 = tmp;
            tmp = tmp->next;
            if (tmp2->data != NULL) os_free(tmp2->data);
...
int list_concat(const list_t *l1, const list_t *l2, list_t *restrict dest) {
    ...
    while (srcel != l1->tail_sentinel) {
        el->next = (struct list_entry_s *)os_malloc(sizeof(struct list_entry_s));
        el->next->prev = el;
        el = el->next;
        el->data = srcel->data;
        srcel = srcel->next;
...
static void list_sort_quicksort(list_t *restrict l, int versus, 
    ...
    if (pivotid > 0) {
        ...
        if (l->threadcount < SIMCLIST_MAXTHREADS-1) {
            struct list_sort_wrappedparams *wp = (struct list_sort_wrappedparams *)os_malloc(sizeof(struct list_sort_wrappedparams));
            l->threadcount++;
            traised = 1;
            wp->l = l;
            wp->versus = versus;
