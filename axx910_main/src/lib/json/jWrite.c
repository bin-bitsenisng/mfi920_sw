...
void modp_int64_toa10(int64_t value, char* str);
...
// - returns current buffer index or zero
//
int jwLen( JWC_DECL0 )
{
	if( (unsigned int)(JWC(bufp) - JWC(buffer)) < JWC(buflen) )
	{
		return (int)(JWC(bufp) - JWC(buffer));
	}
	else
	{
		return 0;
...
void jwObj_int32( JWC_DECL char *key, int32_t value )
{
	modp_itoa10( value, JWC(tmpbuf) );
	jwObj_raw( JWC_PARAM key, JWC(tmpbuf) );
}

void jwObj_int64( JWC_DECL char *key, int64_t value )
{
	modp_int64_toa10( value, JWC(tmpbuf) );
	jwObj_raw( JWC_PARAM key, JWC(tmpbuf) );
}

void jwObj_double( JWC_DECL char *key, double value )
{
	modp_dtoa2( value, JWC(tmpbuf), 9 );
	jwObj_raw( JWC_PARAM key, JWC(tmpbuf) );
}

void jwObj_float( JWC_DECL char *key, float value )
{
	modp_dtoa2( (double)value, JWC(tmpbuf), 6 );
	jwObj_raw( JWC_PARAM key, JWC(tmpbuf) );
}
...
void jwArr_int32( JWC_DECL int32_t value )
{
	modp_itoa10( value, JWC(tmpbuf) );
...
void jwArr_float( JWC_DECL float value )
{
	modp_dtoa2( (double)value, JWC(tmpbuf), 6 );
	jwArr_raw( JWC_PARAM JWC(tmpbuf) );
}
