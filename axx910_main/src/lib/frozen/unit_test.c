...
static const char *test_scanf(void) {
...

  {
    ...
    const char *str = "{\"��\": 123}";
    int x = 0;
    ASSERT(json_scanf(str, strlen(str), "{��: %d}", &x) == 1);
    ASSERT(x == 123);
  }
...
#if 0
  {
    const char *str = "{a : \"極�龜勻筠��\" }";
    char *result;
    ASSERT(json_scanf(str, strlen(str), "{a: %Q}", &result) == 1);
    ASSERT(strcmp(result, "極�龜勻筠��") == 0);
    free(result);
  }
#endif
...
#if 0
  {
    ...
    ASSERT(json_scanf(str, strlen(str), "{a: %V}", &result, &len) == 1);
    ASSERT(len == 14);
    ASSERT(strcmp(result, "極�龜勻筠�귂�") == 0);
    free(result);
  }
#endif
