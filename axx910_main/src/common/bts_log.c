...
#if SYS_CONSOLE_BAUDRATE != 115200 && !defined(WIN32) && !defined(LINUX)
#include "uart_drv.h"
...
pthread_t bts_log_task_handle;
#if BTS_LOG_UDP_EN == 1
uint8_t console_udp_preserve_logbuffer[32*1024];
size_t console_udp_preserve_loglen;
os_stream_t stream_console_udp;
#endif
#endif
...
void bts_log_init(void)
{
    ...
    //krs_memclear(console_udp_preserve_logbuffer);
    //console_udp_preserve_loglen = 0;
#if SYS_CONSOLE_BAUDRATE != 115200 && XPAR_CPU_ID == 0 && !defined(WIN32) && !defined(LINUX)
...
{
#ifndef WIN32
    os_stream_create(stream_console, BTS_LOG_STREAMBUFFER_SIZE, 1);
#if BTS_LOG_UDP_EN == 1
    os_stream_create(stream_console_udp, BTS_LOG_STREAMBUFFER_SIZE, 1);
#endif
...
void bts_log_header(bts_log_type_t type, const char *file, const char *function, int line)
{
    ...
    {
    	offset += snprintf(&consoleheaderbuffer[offset], sizeof(consoleheaderbuffer) - offset, "[");
    	if(BTS_LOG_OPT_DATETIME_EN)
    	{
    		offset += bts_datetime_getstr("%Y-%m-%d:%H:%M:%S",
    				&consoleheaderbuffer[offset],
					sizeof(consoleheaderbuffer) - offset,
					BTS_LOG_OPT_DATETIME_MS_EN,
					BTS_LOG_OPT_DATETIME_US_EN);
    	}
    	else
    	{
    		offset += bts_datetime_getstr("%j:%H:%M:%S",
    				&consoleheaderbuffer[offset],
					sizeof(consoleheaderbuffer) - offset,
					BTS_LOG_OPT_DATETIME_MS_EN,
					BTS_LOG_OPT_DATETIME_US_EN);
...
#if BTS_LOG_UDP_EN == 1
        if((len + console_udp_preserve_loglen) < sizeof(console_udp_preserve_logbuffer))
        {
        	krs_memcopy(console_udp_preserve_logbuffer[console_udp_preserve_loglen], consolebuffer, len);
        	console_udp_preserve_loglen+=len;
        }
#endif
    }
#else
...
void bts_log_task(void *Parameters)
{
    ...
    for(;;)
    {
        ...
        if(ret != 0)
        {
            ...
            printf("%s", csbuff);
#if BTS_LOG_UDP_EN == 1
            if(console_udp_preserve_loglen > 0)
            {
            	os_stream_write(stream_console_udp, console_udp_preserve_logbuffer, console_udp_preserve_loglen, 0);
            	console_udp_preserve_loglen = 0;
            }
            os_stream_write(stream_console_udp, csbuff, ret, 0);
#endif
        }
    }
#endif
...
int32_t bts_log_read(char *buff, size_t size, uint32_t timeoutms)
{
#if BTS_LOG_UDP_EN == 1
	return os_stream_read(stream_console_udp, buff, size, timeoutms);
#else
	return 0;
#endif
}
